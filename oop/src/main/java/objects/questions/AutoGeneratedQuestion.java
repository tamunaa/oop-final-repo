...
public class AutoGeneratedQuestion implements Question, Serializable {
    ...
    public void generateQuestion() {
        ...
        int questionType = random.nextInt(16); // Random number between 0 and 17 (17 types of probability questions)
        switch (questionType) {
            case 0:
    ...
    private void generateProbabilityOfSimpleEventQuestion() {
        ...
        uniqueOptions.add(formattedProbability);

        while (uniqueOptions.size() < 4) {
            double randomOption = random.nextDouble();
            String formattedRandomOption = String.format("%.2f", randomOption);
            uniqueOptions.add(formattedRandomOption);
        }

        options.addAll(uniqueOptions);
        correctOptionIndex = options.indexOf(formattedProbability);
    ...
    private void generateProbabilityOfComplementaryEventQuestion() {
        ...
        Set<String> uniqueOptions = new HashSet<>();
        uniqueOptions.add(formattedProbability);
        while(uniqueOptions.size() < 4)uniqueOptions.add(String.format("%.2f", complementaryProbability * random.nextInt()));

        options.addAll(uniqueOptions);

        // Set the correct option index and shuffle options
        correctOptionIndex = options.indexOf(formattedProbability);
        shuffleOptions(formattedProbability);
    }
    ...
    private void generateProbabilityOfCombinedEventQuestion() {
        ...
        Set<String> uniqueOptions = new HashSet<>();
        uniqueOptions.add(formattedProbability);
        while(uniqueOptions.size()<4)uniqueOptions.add(String.valueOf(random.nextDouble()));
    ...
    private void generateModeQuestion() {
        ...
        // Generate unique options
        Set<String> uniqueOptions = new HashSet<>();
        uniqueOptions.add(modes.toString());
        while(uniqueOptions.size()<4)uniqueOptions.add(setToString(generateRandomSet(5)));

        options.addAll(uniqueOptions);

        // Set the correct option index and shuffle options
        correctOptionIndex = options.indexOf(modes.toString());
    ...
    private void generateUnionQuestion() {
        ...
        // Generate unique options
        Set<String> uniqueOptions = new HashSet<>();
        uniqueOptions.add(setToString(unionSet));
        while(uniqueOptions.size() < 4) uniqueOptions.add(generateRandomSetAsString());

        options.addAll(uniqueOptions);

        // Set the correct option index and shuffle options
        correctOptionIndex = options.indexOf(setToString(unionSet));
        shuffleOptions(setToString(unionSet));
    }


    private String generateRandomSetAsString() {
    ...
    private void generateIntersectionQuestion() {
        ...
        // Generate unique options
        Set<String> uniqueOptions = new HashSet<>();
        uniqueOptions.add(setToString(intersectionSet));
        while(uniqueOptions.size() < 4) uniqueOptions.add(generateRandomSetAsString());

        options.addAll(uniqueOptions);

        correctOptionIndex = options.indexOf(setToString(intersectionSet));
    ...
    private void generateOptions(String correctAnswer) {
        ...
        while (uniqueOptions.size() < 4) {
            ...
            int option = Integer.parseInt(correctAnswer) + (random.nextBoolean() ? offset : -offset);
            uniqueOptions.add(String.valueOf(option));
        }
